...
def makeComponentsFromPath(path):
    return [Component(n.replace(".root",""),os.path.join(path,n)) for n in listdir_uberftp(path) if n.endswith(".root")]
...
comp_SingleMuon2016H_v2 = [makeComponentsFromPath(path) for path in paths_SingleMuon2016H_v2]
